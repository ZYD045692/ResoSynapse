<template>
  <div class="search-section" :id="`section-搜索`">
    <div class="search-buttons">
      <button v-for="platform in props.PlatformList" :key="platform.value" @click="selectPlatform(platform)"
...
// 监听 Platform 数据的变化
watch(() => props.PlatformList, (newPlatformList) => {
  if (newPlatformList.length > 0) {
    const firstItem = newPlatformList[0];
...
}, { immediate: true }); // immediate: true 表示在初始渲染时也执行一次
...
function performSearch() {
  ...
  const platform = props.PlatformList.find(p => p.value === selectedPlatform.value);
  if (platform) {
    const url = `${platform.url}${encodeURIComponent(searchQuery.value)}`;
    window.open(url, '_blank');
  } else {
    ElMessage.error('未找到选中的平台');
  }
}
</script>
